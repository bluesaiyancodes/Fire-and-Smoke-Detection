...
class FireSmokeDetectionApp(QMainWindow):
    ...
    def update_gmm_with_selection_0(self):
    ...
    def update_gmm_with_selection(self):
        ...
        if roi[2] > 0 and roi[3] > 0:
            x, y, w, h = roi
            print(f"ROI selected at x:{x}, y:{y}, width:{w}, height:{h}")
            selected_region = self.current_frame[y:y+h, x:x+w]
            ...
            # Update the GMM models with the new region
            print("Updating GMM models with the selected region.")
            self.gmm_a.apply(a)
            self.gmm_b.apply(b)
            
            # Visual feedback: Show the selected region
